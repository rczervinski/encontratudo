...
export async function POST(req: NextRequest) {
  ...
  const { nome_produto, descricao, preco, preco_promocional, categoria_id, tipo, tags, estoque_quantidade } = body || {}
    ...
    const categoriaId = categoria_id && `${categoria_id}`.trim() !== '' ? parseInt(categoria_id) : null
    const produto = await prisma.produto.create({
      data: { loja_id: lojaId, nome_produto, descricao: descricao || null, preco: precoNumber, preco_promocional: precoPromNumber, categoria_id: categoriaId, tipo: tipo || 'produto', tags: tagsStr, disponivel: true, bloqueado: false, estoque_quantidade: estoque_quantidade != null && `${estoque_quantidade}` !== '' ? Number(estoque_quantidade) : null }
    })
